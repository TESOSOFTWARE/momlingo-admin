...
export const editBabyTrackerList = async (week: string, data: any) => {
  ...
  const response = await axiosInstance.put(`${API_BABY_TRACKERS}`, data);
...
export const newBabyTrackerList = async (data: any) => {
  const response = await axiosInstance.post(`${API_BABY_TRACKERS}`, data);
