...
export const getOrderChart = (params: ISearchForm) => {
  return axiosInstance.get<unknown, IOrdersChart[]>(`${API_ORDER_CHART}/line-chart`, {
    params,
  });
};
export const getOrderChartPie = (params: ISearchForm) => {
  return axiosInstance.get<unknown, { [key in string]: number }>(
    `${API_ORDER_CHART}/overview`,
    { params }
  );
};
...
export const getGameChart = (params: ISearchForm) => {
  return axiosInstance.get<unknown, IGameChart[]>(`${API_GAME_CHART}/line-chart`, {
    params,
  });
};
export const getGameChartPie = (params: ISearchForm) => {
  return axiosInstance.get<unknown, { [key in string]: number }>(
    `${API_GAME_CHART}/overview`,
    { params }
  );
};
