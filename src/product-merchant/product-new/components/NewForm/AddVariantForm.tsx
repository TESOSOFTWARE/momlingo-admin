...
import { addVariantId, defaultVariantId, setDefaultVariantId } from '../../slice';
...
export default function AddVariantForm({
  ...
  useDeepCompareEffect(() => {
    if (detailVariants) {
      setValue('addForm', detailVariants?.name);
    }
  }, [detailVariants]);

  const defaultIdVariant = useSelector(defaultVariantId);
  const listIdVariant = useSelector(addVariantId);
  ...
  return (
    ...
    >
      ...
      <Grid container direction="row" sx={{ paddingLeft: '12px' }}>
        <Grid item xs={12}>
          <RHFTextField name="addForm" variant="standard" disabled />
        </Grid>
        <Grid item xs={3}>
