...
import { useCreateGameGift } from '../../hooks/useCreateGameGift';
...
export default function FormCreateGameGift() {
  ...
  const { mutate } = useCreateGameGift({
    onSuccess: () => {
  ...
  const listProductVirtual =
    dataProductVirtual?.pages
      ?.map((item) =>
        item?.items?.map((itemProd) => {
          return {
            id: itemProd?.id,
            productDetails: itemProd?.productDetails[0],
            name: itemProd?.productDetails[0]?.name,
            thumbnail: itemProd?.thumbnail,
            productVariants: itemProd?.productVariants,
          };
        })
      )
      .flat() || [];
