...
export const RHFSelectPaginationAttributeTerm = ({
  name,
  getAsyncData,
  placeholder,
  searchParams,
  ...
  const loadOptions = async (
    ...
    const optionSelects = response?.items.map((item: any) => {
      return {
        value: item.id,
        label: item.productAttributeTermDetails.map((term: any) => {
          if (item.productAttributeTermDetails.length > 1) {
            return term.value + ' | ';
          } else return term.value;
        }),
      };
    });
