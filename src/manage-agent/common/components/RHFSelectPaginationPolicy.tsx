...
export const RHFSelectPagitnationPolicy = ({
  name,
  getAsyncData,
  placeholder,
  searchParams,
  ...
  const loadOptions = async (
    ...
    const optionSelects = response?.items.map((policy: any) => {
      return {
        value: policy.id,
        label: policy.name,
      };
...
const colourStyles = (isFocus: boolean, error: any) => {
  const styles: StylesConfig = {
    control: (styles, state) => ({
      ...
      border: error?.groupPolicyIds?.message
    ...
    placeholder: (base, state) => ({
      ...
      fontSize:
        (state.hasValue ||
          ...
          (isFocus as unknown as ControlProps<boolean>)) &&
        12,
      color: error?.groupPolicyIds?.message
        ? '#ff4842!important'
        : (isFocus as unknown as ControlProps<boolean>)
        ? '#00ab55'
        : state.hasValue || state.selectProps.inputValue
