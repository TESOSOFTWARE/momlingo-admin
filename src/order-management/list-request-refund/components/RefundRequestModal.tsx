...
export default function RefundRequestModal(prop: Props) {
  ...
  const { paramsRefundRequest, reject } = useSelector(
    (state) => state.refundOrderRequest
  );
  ...
  const { mutate: mutateReject, isSuccess: isSuccessReject } =
    useRejectRefund(paramsRefundRequest);
  useEffect(() => {
        ...
        <FormProvider methods={methods} onSubmit={handleSubmit(onSubmitForm)}>
          ...
          <DialogContent sx={{ marginTop: '20px' }}>
            <Stack spacing={3}>
              ...
              <RHFTextField
                label={!reject ? 'Nội dung hoàn xu' : 'Nội dung từ chối'}
                InputLabelProps={{ shrink: true }}
                name="content"
              />
            </Stack>
