...
export const RHFSelectPaginationShop = ({
  name,
  getAsyncData,
  placeholder,
  searchParams,
  ...
  const loadOptions = async (
    ...
    const responseWithStatus1 = response?.data.shops.filter(
      (item: any) => item.status === 1
    );

    const hasMore = page < responseWithStatus1?.length / 10;

    const optionSelects = responseWithStatus1?.map((item: any) => {
      return {
